...
def train_transforms(img, left_bd, upper_bd, right_bd, lower_bd):
    img = img.crop((left_bd, upper_bd, right_bd, lower_bd))
...
@dataclass(frozen=True)
class DatasetConfig:
    def __init__(self, data_root='D:/Side Projects D/repo/TCG Shop Stream OCR/Solution1/OCR Pretrains/imgs/'):
...
def OCR_finetune(model_name, train_imgs_root, train_labels, device, max_target_length, left_bd, upper_bd, right_bd, lower_bd) -> None:
    processor = TrOCRProcessor.from_pretrained(model_name)
    # device = torch.device(device)
    df = pd.read_csv(train_labels, sep=" ", header=None, names=["ssID", "Label"], index_col=None)
    train_dataset = CustomOCRDataset(
        root_dir=os.path.join(train_imgs_root),
        df=df,
        processor=processor,
    ...
    """
    # Evaluation Metric and Inner Function
...
def main(processor="microsoft/trocr-small-printed", 
         train_imgs_root='D:/Side Projects D/repo/TCG Shop Stream OCR/Solution1/OCR Pretrains/imgs/', 
         train_labels="OCR Pretrains/imgs_labels.txt", 
         device='cpu',
         max_target_length=10,
         left_bd=460, upper_bd=260, right_bd=580, lower_bd=290) -> int:
    OCR_finetune(processor, train_imgs_root, train_labels, device, max_target_length, left_bd, upper_bd, right_bd, lower_bd)
    return 0
